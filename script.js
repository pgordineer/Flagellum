...
function saveHighScores() {
  ...
  // Saviour mode: lowest actions to win is best (so lower is better, but only if you win)
  // Only update if saviourGameOver is true and you won (not eliminated)
  if (
    saviourGameOver &&
    isSaviourWin() &&
    (
...
// Helper to check if current saviour state is a win
function isSaviourWin() {
  let activeCount = 0, lastActiveIdx = -1;
  ...
  return (activeCount === 1 && lastActiveIdx === saviourHighlightIndex);
}
...
function renderSaviourGrid(gameOver = false) {
  ...
  if (activeCount === 1 && lastActiveIdx === saviourHighlightIndex && !gameOver) {
    ...
    // Save high score for lowest actions to win
    if (
      saviourHighScore === 0 ||
      saviourScore < saviourHighScore ||
      (saviourScore === saviourHighScore && saviourGrid.length < saviourHighTotal)
    ) {
      saviourHighScore = saviourScore;
...
// --- Saviour Mode High Score: lowest actions to win (not highest) ---
function saveHighScores() {
  // Entry mode
  if (
    entryScore > entryHighScore ||
...
function gammaBurstAction(idx) {
  processSaviourActionElimination(idx, (cb) => {
    for (let i = 0; i < saviourGrid.length; i++) {
